...
class ESP32HumanDetectionSystem:
    def __init__(self, esp32_url="http://192.168.20.152:81/stream", 
        ...
        # Recording settings
        self.RECORDING_DELAY = 10  # 4 seconds delay before stopping recording
        
        # --- MODIFIKASI: Pengaturan Pre-detection Buffer ---
        self.PRE_DETECTION_BUFFER_SECONDS = 3.0  # Simpan 5 detik sebelum deteksi
        self.ESTIMATED_FPS_FOR_BUFFER = 20.0     # Asumsi FPS untuk kalkulasi ukuran buffer
        buffer_size = int(self.PRE_DETECTION_BUFFER_SECONDS * self.ESTIMATED_FPS_FOR_BUFFER)
        self.pre_detection_buffer = deque(maxlen=buffer_size)
        self.buffer_lock = threading.Lock()
        logger.info(f"Pre-detection buffer initialized for ~{self.PRE_DETECTION_BUFFER_SECONDS} seconds (size: {buffer_size} frames)")
        # --- AKHIR MODIFIKASI ---

        # Camera control defaults
        self._default_camera_settings = [
            ("vflip", 1),
            ("hmirror", 1),
            ("framesize", 10),
            ("gainceiling", 1),
    ...
    def _derive_control_base_url(self):
        ...
        try:
            parsed = urllib.parse.urlparse(self.esp32_url)
            ...
            if port in (None, 80) and scheme == 'http':
                netloc = hostname
            ...
            logger.warning(f"Unable to derive control URL from '{self.esp32_url}': {exc}")
            return None
    ...
    def _configure_camera(self):
        ...
        for var, val in self._default_camera_settings:
            control_url = f"{self._control_base_url}/control?var={var}&val={val}"
            try:
                with urllib.request.urlopen(control_url, timeout=3) as response:
    ...
    def _start_recording(self):
        ...
        self.video_writer = cv2.VideoWriter(filepath, fourcc, self.ESTIMATED_FPS_FOR_BUFFER, (width, height))
    ...
    def _recording_thread(self):
        ...
        while self.running:
            try:
                ...
                time.sleep(1 / self.ESTIMATED_FPS_FOR_BUFFER)
...
def main():
    ...
    print("=" * 60)
    print("ESP32-CAM Human Detection System (with Pre-Detection Buffer)") # Teks diubah
    print("=" * 60)
    ...
    print("- Video now includes ~5 seconds BEFORE detection.") # Teks ditambah
    print("=" * 60)
